...
const uint16_t red_channel = 7;
const uint16_t green_channel = 8;
const uint16_t blue_channel = 9;
...
public:
	...
	~Drums() {
		...
		stop();
		// Clear artnet before quitting
		for (uint16_t i = 0; i < 512; ++i) {
			artnet_packet_[i] = 0;
		}
		artnet_packet_.send(socket_);
	}
private:
	...
	artnet::DatagramSocket socket_;
	artnet::Packet artnet_packet_;
	/**
	...
	void update_screen()
	{
		...
		artnet_packet_[red_channel] = color.r;
		artnet_packet_[green_channel] = color.g;
		artnet_packet_[blue_channel] = color.b;
		artnet_packet_.send(socket_);
